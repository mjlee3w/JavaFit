...
public class FileAccess {
  ...
  public static EnumMap<Config.MuscleGroup, ArrayList<Config.Muscle>> loadFormats() {
    ...
    try
    {
      Scanner sc = new Scanner(new File(Config.WORKOUTFORMATFILE));
      while(sc.hasNextLine())
      {
        String line = sc.nextLine();
        String[] fields = line.split(",");
        Config.MuscleGroup mGroup = Config.MuscleGroup.valueOf(fields[0]);
        ArrayList<Config.Muscle> m = new ArrayList <Config.Muscle>();
        for(int i = 1; i < fields.length; i++)
        {
          m.add(Config.Muscle.valueOf(fields[i]));
        }
